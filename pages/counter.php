...
$counter_values = dbSelect($dbconnect, "SELECT cv.id as id, cv.value as value, cv.date as date FROM counter_values cv INNER JOIN counters c ON c.id=cv.counter_id INNER JOIN flats f ON f.id=c.flat_id WHERE f.user_id='".$userinfo['id']."' AND counter_id='".$counterid."' ORDER BY cv.date;");
...
{
	echo "<tr><td>".$counter_value['date']."</td><td>".$counter_value['value']."</td><td><form action=\"index.php?page=counter&id=".$counterid."\" method=\"POST\"> <input type=\"hidden\" name=\"action\" value=\"delete\"> <input type=\"hidden\" name=\"vaue_id\" value=\"".$counter_value['id']."\"> <input type=\"submit\" name=\"submit\" value=\"Удалить\"></form></td></tr>";
}
